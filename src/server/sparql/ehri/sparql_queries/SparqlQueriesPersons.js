export const personsProperties = `
  {
    ...
    BIND(CONCAT("https://portal.ehri-project.eu/authorities/ehri_pers-", REPLACE(STR(?id), "^.*\\\\/(.+)", "$1")) AS ?portalURI__prefLabel)
    BIND(CONCAT("https://portal.ehri-project.eu/authorities/ehri_pers-", REPLACE(STR(?id), "^.*\\\\/(.+)", "$1")) AS ?portalURI__dataProviderUrl)
  }
  UNION
  {
  ...
  UNION
  {
    ?id rico:hasBirthDate/rico:normalizedDateValue ?birthDate .
  }
  UNION
  {
    ?id rico:hasDeathDate/rico:normalizedDateValue ?deathDate .
  }
  UNION
  {
  ...
  UNION
  {
    ?id ehri:structureOrGenealogy ?genealogy .
  }
  UNION
  {
...
export const personsPlusLinkedDescriptionsProperties = personsProperties + `
  BIND(CONCAT("/sampo/en/archivalDescriptions/faceted-search/table?page=0&constraints=%5B%7B%22facetClass%22%3A+%22archivalDescriptions%22%2C+%22facetId%22%3A%22ehriPersons%22%2C%22filterType%22%3A%22uriFilter%22%2C%22value%22%3A%7B%22node%22%3A%7B%22id%22%3A%22",  STR(?id), "%22%2C%22prefLabel%22%3A%22", STR(?prefLabel__id), "%22%7D%7D%7D%5D") AS ?archivalDescriptionsLink)
  BIND(?archivalDescriptionsLink AS ?archivalDescriptionsLink__dataProviderUrl)
  BIND("Use this person as filter in the archival descriptions perspective" AS ?archivalDescriptionsLink__prefLabel)
`
